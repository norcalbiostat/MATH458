...
**without replacement**
```{r}
pop.idx <- 1:NROW(stats10) # 1, 2
(idx <- sample(pop.idx, 4)) #3 
stats10[idx,] #4
```
...
The functions `srswr(n,N)` and `srswor(n,N)` draw SRS with, and without replacement respectively. Each take two arguments: $n$ the sample size, and $N$ the population size, and what is returned is a vector of length $N$ that indicates how many times that position in the vector is selected.

```{r}
set.seed(4134)
(choose.these.wor <- srswor(4, 10)) 
...
```{r}
getdata(stats10, choose.these.wor)
getdata(stats10, choose.these.wr)
```

* Again, notice Boston was chosen twice when sampling with replacement. 
...
n <- 300; N <- NROW(agpop) # i'm storing these as constants here because we'll use them later. 
set.seed(8675309)
ag.idx <- srswor(n, N)
ag.srs <- getdata(agpop, ag.idx)
head(ag.srs)
```
...
* So the variance is more determined by sample size, not the % of the population sampled. 
...
Sample means generated from samples of size ______ will vary from sample to sample by _____ acres. 
...
In an SRS, each unit has an inclusion probability of $\delta_{i} = $, so the sampling weights are all $w_{i} =$. 
...
```{r}
svymean(~_______+_______, ag.srs.dsgn)
```

You can pass the results to construct confidence intervals. 
```{r}
_______(~acres92+farms92, ag.srs.dsgn) |> _______()
```
