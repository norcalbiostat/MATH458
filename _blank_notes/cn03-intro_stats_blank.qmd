...
```{r}
# delete this line and write your own code
```
...
```{r}
samp.unif <- runif(___, __, __) # runif(n, min, max)
mean(samp.unif)
```
...
> if this doesn't work for you, then make sure you have the correct data set downloaded into your correct `data` folder. 
...
Hooray! We have just examined a _sampling distribution_ of the sample mean. 
...
```{r}
sd(____) #because the example above already had n=10
...
> If the base pipe |> doesn't work for you, it may be because you don't have the newest version of R. An alternative is to assign the results of `replicate` to an object, then call `sd()` on that object. Delete this line after you successfully run the code above. 
...
```{r}
(LCL <- ____(many.means) - 2*____(many.means))
(UCL <- ____(many.means) + 2*____(many.means))
```

```{r}
LCL <- UCL <-1  # this code chunk is only done so these notes will compile. Delete it once you fill in the blanks above. 
```
