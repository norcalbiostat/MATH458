...
universe$selection.prob <- rep(deltas,4)*rep(deltas, each=4)
universe
```

Note the resulting object is a table/matrix with 16 rows and 3 columns (currently named `Var1` and `Var2` and `selection.prob`), where each row is a possible sample of $n=2$ from the universe. 

Create a function to calculate $\hat{\tau} = \frac{1}{2}\Big[\frac{x_1}{\delta_1} + \frac{x_2}{\delta_2}\Big]$. 

```{r}
get.taus <- function(x1, x2){
  .5*(x1/deltas[x1] + x2/deltas[x2])
}
...
```{r}
all.tau.hat <- get.taus(universe$Var1, universe$Var2)
head(all.tau.hat) # always check your code at each step
```

Now calculate $E[\hat{\tau}] = \sum \hat{\tau}*p(\hat{\tau})$ and $V[\hat{\tau}] = E[\hat{\tau}] - E[\hat{\tau}]^2]. 

```{r}
(E.tau.hat <- sum(all.tau.hat * universe$selection.prob))
(Var.tau.hat <- sum(all.tau.hat^2 * universe$selection.prob) - E.tau.hat^2)
```
