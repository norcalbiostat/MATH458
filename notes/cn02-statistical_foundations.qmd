...
These notes require the [StatisticsPhD.csv](`r here::here("data", "StatisticsPhD.csv")`) data file.

```{r}
stat.phd <- readr::read_csv(here::here("data", "StatisticsPhD.csv"))
```
_Note: I am using the `here` package and an [Rstudio project](https://support.posit.co/hc/en-us/articles/200526207-Using-RStudio-Projects) to manage my files and data. This is a recommended way to manage the files related to this class. If you choose not to, then you will have to modify this code to read in your own path._
:::
...
Along with the point estimate we also want to know \_\_\_\_\_\_\_\_\_ we can expect the point estimate to be. In other words, if we took another random sample of the same size from the population, is the point estimate from this new sample ......

::: {.callout-tip icon="true"}
### Example: Enrollment in Graduate Programs in Statistics

One factor that can come into play when choosing a graduate program is the size of the program. This can help determine how much support and funding may be available. There are 82 US statistics doctoral programs for which enrollment data were available. The data set `StatisticsPhD` lists all these schools together with the total enrollment of full-time graduate students in each program in 2009.
...
Based on the data set, the mean enrollment in 2009 is \_\_\_\_\_\_\_\_ full-time graduate students. Because this is the mean for the entire population of all US statistics doctoral programs for which data were available that year, we have that $\mu$ =\_\_\_\_\_\_\_ students.
...
🎉 We have just created a *\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_* of the sample mean.
...
-   **Estimation bias** means that the estimator chosen resulted in _______.
...
If two unbiased estimators are available for $\theta$ we generally prefer the one with _______.

Because sometimes we use biased estimators, we often use the _______ instead of the variance to estimate the _______..
...
-   A is _______ The average position of all arrows is at the center of the target.
-   B is _______ but not unbiased. All arrows are close together but systematically away from the center.
-   C is _______ All arrows are close together and near the center of the target.
...
Estimate the standard error for the mean enrollment in statistics PhD programs for a sample size of 10 and also a sample size of 20. [I used the base pipe `|>` here to pass the results of the replicate function into the `sd()` function]{.aside}
:::
...
When using a formula to calculate an approximate 95% confidence interval, use \_\_\_\_\_\_\_\_\_\_\_\_\_\_ as the margin of error.
...
> We can be 95% confident that PhD programs in Statistics have a total enrollment of between \_\_\_\_\_\_ and \_\_\_\_\_\_ students.
