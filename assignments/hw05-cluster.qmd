...
```{r}
#| eval: false
measles <- readr::read_csv(here::here("data","measles.csv"))
```
...
> Helper code: We separated the returned consent forms from the forms with no reply by setting our returned set as `as.numeric(returnf==1)`.

```{r}
#| eval: false
measles$returned <- as.numeric(measles$returnf==1)
```

b.  Using the number of respondents in school $i$ as $m_i$, construct the sampling weight for each observation. *(Hint: Use the variables  listed as `measles$mi` and `measles$Mitotal` in our data set for this question)*

c.  Estimate the overall percentage of parents who received a consent form along with a 95% CI.

d.  How do your estimate and interval in part (c.) compare with the results you would have obtained if you had ignored the clustering and analyzed the data as an SRS? Find the ratio: $$\frac{\text{estimated variance from (c)}}{\text{estimated variance if the data were analyzed as an SRS}}$$ What is the effect of clustering?

# D. Projects and Activities - Baseball Data 

a. Use the population in the file `baseball.csv` to take a one-stage cluster sample with the teams as the psus. Your sample should have approximatley 150 players altogether, as in the SRS from Exercise 37 of Chapter 2. Describe how you selected your sample. Don't forget to set a seed. 
...
srs_sample <- baseball[sample(nrow(baseball), 150), ]

mean_logsal <- mean(srs_sample$logsal, na.rm = TRUE)
se_logsal <- sd(srs_sample$logsal, na.rm = TRUE) / sqrt(sum(!is.na(srs_sample$logsal)))
ci_logsal <- c(mean_logsal - 1.96 * se_logsal, mean_logsal + 1.96 * se_logsal)

is_pitcher <- grepl("P", srs_sample$pos, ignore.case = TRUE)
p_hat <- mean(is_pitcher, na.rm = TRUE)
se_p <- sqrt(p_hat * (1 - p_hat) / sum(!is.na(is_pitcher)))
ci_pitchers <- c(p_hat - 1.96 * se_p, p_hat + 1.96 * se_p)
```
